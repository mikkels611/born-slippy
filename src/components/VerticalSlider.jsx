...
export default memo(function VerticalSlider({ label, value, onChange, min=0, max=1, color="#e05020", muted, onMute, solo, onSolo, soloActive, rec, onRec, isDark=true, log=false }) {
  ...
  const onTouchStart = useCallback((e) => {
    ...
    const v = applyVisual(e.touches[0].clientY);
    onChangeRef.current(v);
    const mv = (ev) => { ev.preventDefault(); const val = applyVisual(ev.touches[0].clientY); onChangeRef.current(val); };
    const up = () => { dragging.current = false; rectCache.current = null; document.removeEventListener("touchmove", mv); document.removeEventListener("touchend", up); };
  ...
  const onMouseDown = useCallback((e) => {
    dragging.current = true;
    rectCache.current = trackRef.current.getBoundingClientRect();
    const v = applyVisual(e.clientY);
    onChangeRef.current(v);
    const mv = (ev) => { const val = applyVisual(ev.clientY); onChangeRef.current(val); };
    const up = () => { dragging.current = false; rectCache.current = null; document.removeEventListener("mousemove", mv); document.removeEventListener("mouseup", up); };
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:4, flex:1, minWidth:0 }}>
      {onRec && <button onClick={onRec} style={{ width:32, height:20, borderRadius:3, background:recBtnBg, border:`1px solid ${recBtnBorder}`, color:recBtnColor, fontSize:7, fontWeight:700, letterSpacing:0.5, fontFamily:"'Space Mono', monospace", cursor:"pointer", WebkitTapHighlightColor:"transparent", padding:0 }}>R</button>}
      ...
      >
        <div ref={fillRef} style={{ position:"absolute", bottom:0, left:0, right:0, height:`${norm*100}%`, borderRadius:19, background:`linear-gradient(to top, ${dc}44, ${dc}22)` }} />
        <div ref={knobRef} style={{ position:"absolute", left:"50%", bottom:`calc(${norm*100}% - 12px)`, transform:"translateX(-50%)", width:24, height:24, borderRadius:"50%", background:knobBg, border:`2px solid ${dc}`, boxShadow:`0 0 6px ${dc}33` }}>
      ...
      {onMute && <button onClick={muteBlocked?undefined:onMute} style={{ width:32, height:20, borderRadius:3, background:muteBtnBg, border:`1px solid ${muteBtnBorder}`, color:muteBtnColor, fontSize:7, fontWeight:700, letterSpacing:0.5, fontFamily:"'Space Mono', monospace", cursor:muteBlocked?"default":"pointer", WebkitTapHighlightColor:"transparent", padding:0, opacity:muteBlocked?0.25:1 }}>{muted?"OFF":"M"}</button>}
      {onSolo && <button onClick={onSolo} style={{ width:32, height:20, borderRadius:3, background:soloBtnBg, border:`1px solid ${soloBtnBorder}`, color:soloBtnColor, fontSize:7, fontWeight:700, letterSpacing:0.5, fontFamily:"'Space Mono', monospace", cursor:"pointer", WebkitTapHighlightColor:"transparent", padding:0 }}>S</button>}
    </div>
  );
});
